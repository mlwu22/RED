...
parser.add_argument("--model_type", default="gpt2-large")
...
meta_dir = f"{main_dir}/Results/RED++/gpt2-large/[Substitute_meta]/[Substitute_dataset]/lr_{str(lr)}/weight_dacay_{str(weight_decay)}/seed_{str(seed)}/label_smooth_{str(label_smooth)}"
...
lr_scheduler = get_scheduler("linear", optimizer=optimizer, num_warmup_steps=warmup_step, num_training_steps=total_train_steps)
...
if(do_train):
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataLoader)):
            ...
            loss = calculate_loss(logits, labels, tokenizer.pad_token_id, shift=True, label_smoothing=label_smooth)
